...
for xi = 1 : N
    ...
    if(xi<5)
        ...
        [~,index] = max(sum(abs(levelset-[levelset(2:end,:);levelset(1,:)]),2).*sum(abs(e-[e(2:end,:);e(1,:)]).^0.6,2));
        yi = index + 1;
        if(index == xi-1)
...
%This part can take very long time, often 10 min, please wait patiently
...
onpoints = zeros(4,2);
%Max Length for judge if the point is on the nonsmooth line
ML = 6;
for k = 1:size(Vc_v,2)
    for i = 1:N
    ...
    vb = find(cont_dist>ML);
    % update the insert points
    for j = 1:size(vb,1)
        vf = find(insert_index == vb(j));
        if(isempty(vf) && size(insert_index,2)<20)
    ...
    for i = 1:size(vI,2)
        % only update the active insert_points
        if sum((cont_final(insert_index(vI(i)),:)-cont_final((insert_index(vI(i))+1),:)).^2)>ML
...
boundary_spiral = boundary_spiral(1:2920,:);
c_bsp = boundary_spiral(4*(1:size(boundary_spiral,1)/4-1)+2,:);
boundary_spiral = boundary_spiral(1:1980,:);
a_bsp = boundary_spiral(4*(1:size(boundary_spiral,1)/4-1),:);
% This is very special, for control constrained case the nonsmooth line has
% a "H" structure which cannot be caught by the algorithm
a_bsp = [a_bsp;3.12444,0.011;3.10844,-0.109;3.10844,-0.11;3.12444,0.01];
...
function dxdt = odefunc_pen_control_min(x,m,l,b,g,R,F,mu,bsp1,bias)
    ...
    % bias = 0;
    % while (abs(x1)>pi)
    %     x1 = x1 - x1/abs(x1)*2*pi;
    % end
    % [in,~] = inpolygon(x1,x2,bsp1(:,1),bsp1(:,2));
    % if(in ~= 1)
    %    [in,~] = inpolygon(x1+2*pi,x2,bsp1(:,1),bsp1(:,2));
    %    if(in == 1)
    %        bias = 2*pi;
    %    else
    %        bias = -2*pi;
...
function dxdt = odefunc_pen_sat_varify(t,x,m,l,b,g,q,R,F2,mu)
    dxdt = zeros(3,1);
    x1 = x(1);
    x2 = x(2); 
    j2 = F2(x1,x2);
    ...
    dxdt(1) = x2;
    dxdt(2) = (-x2*b/m/l^2+g/l*sin(x1)+u/m/l^2);
    dxdt(3) = q(1)*(2-2*cos(x1))+q(2)*x2^2+R*(u)^2;
end

function dxdt = odefunc_para(x,a,b)
    dxdt = 1/sqrt(a^2*cos(x)^2+b^2*sin(x)^2);
end
...
function [value,isterminal,direction] = odeEventFun_test(t,y,H1)
    value=y(1)^2+y(2)^2 - 1; %test if done
    isterminal=1;   
    direction=1;     
end

function [value,isterminal,direction] = paraEventFun(y)
    value=y-2*pi; %test if done
    isterminal=1;   
    direction=1;     
end
...
function parsave(i,y)
    save(strcat(num2str(i)),'y','-nocompression');
    %save('data.mat','y','-nocompression','-append');
end
